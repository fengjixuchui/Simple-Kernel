...
//  bitmap_bytemirror(swapped_image, 12, 27, swapped_image2);
  ...
  single_char_anywhere_scaled(LP->GPU_Mode->FrameBufferBase, swapped_image, 24, 27, LP->GPU_Mode->Info->HorizontalResolution, LP->GPU_Mode->Info->VerticalResolution, 0x0000FFFF, 0xFF000000, (LP->GPU_Mode->Info->HorizontalResolution - 5*27)/2, (LP->GPU_Mode->Info->VerticalResolution - 5*24)/2, 5);
...
void bitmap_bitswap(char * bitmap, UINT32 height, UINT32 width, char * output)
{
  ...
  for(int iter = 0; iter < height*row_iterator; iter++) // flip one byte at a time
  {
